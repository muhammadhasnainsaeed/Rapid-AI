...
export default function GenerateImages() {
  ...
  const [selectedStyles, setSelectedStyles] = useState(imageStyles[0]);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate an image of ${input} in the style ${selectedStyles}`;
      ...
      >
        ...
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3 flex-wrap md:max-w-9/11">
          {imageStyles.map((item, index) => (
            <span
              key={index}
              onClick={() => setSelectedStyles(item)}
        ...
        <div class="my-6 flex items-center gap-2">
          <label class="relative cursor-pointer">
            <input
              ...
              type="checkbox"
            />
            <div class="w-9 h-5 bg-slate-300 rounded-full peer-checked:bg-green-500 transition"></div>
            <span class="absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition peer-checked:translate-x-4"></span>
          </label>
          <p class="text-sm">Make this image Public</p>
        </div>
        <button
          disabled={loading}
          type="submit"
